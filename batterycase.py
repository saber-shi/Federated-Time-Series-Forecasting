...
def _ensure_gurobi() -> None:
	if gp is None:
		raise ImportError(
			"gurobipy is required for battery optimization. Install with `pip install gurobipy` "
...
def _demo_data(n_stations: int = 3, n_slots: int = 24) -> tuple[pd.DataFrame, np.ndarray, list[BatterySpec]]:
	...
	prices = np.array([0.10] * 7 + [0.18] * 5 + [0.30] * 6 + [0.16] * 6, dtype=float)
...
def main(argv: Iterable[str] | None = None) -> None:
	...
	parser.add_argument(
		...
		type=str,
		default=None,
		help="Path to CSV containing one price column with same number of rows as loads_csv.",
	)
	parser.add_argument("--price_col", type=str, default=None, help="Price column name when using --prices_csv.")
